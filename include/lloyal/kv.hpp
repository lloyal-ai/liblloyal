...
inline bool remove_range(llama_context* ctx, llama_seq_id seq, llama_pos p0, llama_pos p1) {
  ...
  if (!success) {
    LLOYAL_LOG_DEBUG("[kv::remove_range] FAILED: seq=%d, p0=%d, p1=%d", seq, p0, p1);
    LLOYAL_LOG_DEBUG("[kv::remove_range] Guard-rail reminder: Ensure remove_range called BEFORE next llama_decode()");
  } else {
    LLOYAL_LOG_DEBUG("[kv::remove_range] OK: seq=%d, removed tokens [%d, %d)", seq, p0, p1);
  }
...
inline size_t state_size(llama_context* ctx, llama_seq_id seq) {
  ...
  llama_pos max_pos = llama_memory_seq_pos_max(mem, seq);
  if (max_pos < 0) {
    LLOYAL_LOG_DEBUG("[kv::state_size] WARNING: KV cache is empty (max_pos=%d) - returning 0", max_pos);
    return 0;
  }
  ...
  if (size == 0) {
    LLOYAL_LOG_DEBUG("[kv::state_size] Per-sequence size query failed for seq=%d", seq);
    LLOYAL_LOG_DEBUG("[kv::state_size] Attempting global state size (fallback)");
    size = llama_state_get_size(ctx);

    if (size > 0) {
      LLOYAL_LOG_DEBUG("[kv::state_size] Global fallback size: %zu bytes", size);
    } else {
      LLOYAL_LOG_DEBUG("[kv::state_size] ERROR: Both per-sequence and global size queries failed");
    }
  } else {
    LLOYAL_LOG_DEBUG("[kv::state_size] Per-sequence size for seq=%d: %zu bytes (%.1f MB)",
...
inline size_t state_save(llama_context* ctx, llama_seq_id seq, uint8_t* dst, size_t size) {
  if (!ctx || !dst || size == 0) {
    LLOYAL_LOG_DEBUG("[kv::state_save] ERROR: invalid parameters (ctx=%p, dst=%p, size=%zu)",
              ctx, dst, size);
    return 0;
  }
  ...
  llama_pos max_pos = llama_memory_seq_pos_max(mem, seq);
  if (max_pos < 0) {
    LLOYAL_LOG_DEBUG("[kv::state_save] WARNING: KV cache is empty (max_pos=%d) - skipping save", max_pos);
    return 0;
  }
  ...
  if (written == 0) {
    LLOYAL_LOG_DEBUG("[kv::state_save] Per-sequence save failed for seq=%d (possible KV fragmentation)", seq);
    LLOYAL_LOG_DEBUG("[kv::state_save] Attempting global state save (fallback)");
    written = llama_state_get_data(ctx, dst, size);

    if (written > 0) {
      LLOYAL_LOG_DEBUG("[kv::state_save] Global fallback succeeded: %zu bytes (%.1f MB)",
                written, written / 1024.0 / 1024.0);
    } else {
      LLOYAL_LOG_DEBUG("[kv::state_save] ERROR: Both per-sequence and global save failed");
    }
  } else {
    LLOYAL_LOG_DEBUG("[kv::state_save] Per-sequence saved %zu bytes (%.1f MB) for seq=%d",
...
inline size_t state_load(llama_context* ctx, llama_seq_id seq, const uint8_t* src, size_t size) {
  if (!ctx || !src || size == 0) {
    LLOYAL_LOG_DEBUG("[kv::state_load] ERROR: invalid parameters (ctx=%p, src=%p, size=%zu)",
              ctx, src, size);
    return 0;
  }
  ...
  llama_pos max_pos = llama_memory_seq_pos_max(mem, seq);
  if (max_pos < 0) {
    LLOYAL_LOG_DEBUG("[kv::state_load] WARNING: KV cache is empty (max_pos=%d) - loading may crash on recurrent models", max_pos);
  }

  size_t read = llama_state_seq_set_data(ctx, src, size, seq);

  if (read == 0) {
    LLOYAL_LOG_DEBUG("[kv::state_load] Per-sequence restore failed for seq=%d (possible fragmentation)", seq);
    LLOYAL_LOG_DEBUG("[kv::state_load] Attempting global state restore (fallback)");
    read = llama_state_set_data(ctx, src, size);

    if (read > 0) {
      LLOYAL_LOG_DEBUG("[kv::state_load] Global fallback succeeded: %zu bytes (%.1f MB)",
                read, read / 1024.0 / 1024.0);
    } else {
      LLOYAL_LOG_DEBUG("[kv::state_load] ERROR: Both per-sequence and global restore failed");
    }
  } else {
    LLOYAL_LOG_DEBUG("[kv::state_load] Per-sequence loaded %zu bytes (%.1f MB) for seq=%d",
...
inline size_t global_state_size(llama_context* ctx) {
  ...
  LLOYAL_LOG_DEBUG("[kv::global_state_size] %zu bytes (%.1f MB)", size, size / 1024.0 / 1024.0);
  return size;
}
...
inline size_t global_state_load(llama_context* ctx, const uint8_t* src, size_t size) {
  ...
  LLOYAL_LOG_DEBUG("[kv::global_state_load] %zu bytes read (%.1f MB)",
            read, read / 1024.0 / 1024.0);
  return read;
}
...
inline void log_build_info(llama_context* ctx) {
  LLOYAL_LOG_DEBUG("[kv::build_info] ============================================");
  LLOYAL_LOG_DEBUG("[kv::build_info] llama.cpp KV Sequence Operations Configuration");
  LLOYAL_LOG_DEBUG("[kv::build_info] ============================================");
  LLOYAL_LOG_DEBUG("[kv::build_info] Version: b6870");
  LLOYAL_LOG_DEBUG("[kv::build_info] API naming: llama_memory_seq_*");
  LLOYAL_LOG_DEBUG("[kv::build_info] Current MVP: n_seq_max=1 (single sequence only)");

  if (ctx) {
    llama_pos max_pos = pos_max(ctx, 0);
    if (max_pos >= 0) {
      LLOYAL_LOG_DEBUG("[kv::build_info] Current KV cursor (seq 0): %d tokens", max_pos);
    } else {
    ...
    if (snapshot_size > 0) {
      LLOYAL_LOG_DEBUG("[kv::build_info] Estimated snapshot size: %zu bytes (%.1f MB)",
                snapshot_size, snapshot_size / 1024.0 / 1024.0);
    }
  }

  LLOYAL_LOG_DEBUG("[kv::build_info] Fragmentation fallback: per-sequence → global state");
  LLOYAL_LOG_DEBUG("[kv::build_info] Critical: Call remove_range() BEFORE llama_decode()");
  LLOYAL_LOG_DEBUG("[kv::build_info] ============================================");
}
