...
TEST_CASE("Sampler: sample_with_params - greedy sampling (temp=0, seed deterministic)") {
    resetStubConfig();
...
// NOTE: Grammar sampling tests require real llama.cpp and are in integration/Sampler.cpp
// Stubs cannot meaningfully test grammar sampling because llama_sampler_apply() logic
// requires actual parser state, which is too complex to mock accurately.
