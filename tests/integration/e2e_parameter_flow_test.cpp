 ...
 *
 * Verifies complete parameter flow from model loading -> context init -> inference
 * These tests ensure that user-provided params correctly affect the entire inference
 * pipeline, not just individual components.
 *
...
TEST_CASE("E2E: Complete inference pipeline with custom parameters") {
    ...
    // Add tokens to batch
    batch.n_tokens = static_cast<int32_t>(std::min(tokens.size(), size_t(ctx_params.n_batch)));
    for (int32_t i = 0; i < batch.n_tokens; ++i) {
        batch.token[i] = tokens[i];
        batch.pos[i] = i;
        batch.n_seq_id[i] = 1;
        batch.seq_id[i][0] = 0;  // Sequence 0
        batch.logits[i] = (i == batch.n_tokens - 1) ? 1 : 0;  // Only last token needs logits
    }
    ...
    llama_token next_token = llama_sampler_sample(sampler, ctx, batch.n_tokens - 1);
    ...
    INFO("E2E test completed: model params + context params + inference successful");
}
