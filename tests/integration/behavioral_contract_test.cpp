...
TEST_CASE("Behavioral: Tokenization produces consistent token IDs") {
    ...
    if (tokens.size() != EXPECTED_TOKEN_COUNT) {
        WARN("Token count changed! This may indicate llama.cpp tokenization changed.");
...
TEST_CASE("Behavioral: KV cache state size is stable") {
    ...
    if (state_size < EXPECTED_MIN_STATE_SIZE || state_size > EXPECTED_MAX_STATE_SIZE) {
...
TEST_CASE("Behavioral: Detokenization produces consistent text") {
    ...
    std::string text1 = tokenizer::detokenize_batch(
        vocab, tokens.data(), tokens.size(), false, false
    );

    std::string text2 = tokenizer::detokenize_batch(
        vocab, tokens.data(), tokens.size(), false, false
    );
