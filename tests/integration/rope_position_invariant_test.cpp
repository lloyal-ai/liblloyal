 ...
 *
 * CRITICAL INVARIANT: After clear+reseed, KV cache positions MUST be contiguous.
 *
 * StreamingLLM Paper (Section 3.2):
 * > "cache the Keys of tokens prior to introducing the rotary transformation. Then, we apply
 * > position transformation to the keys in the rolling cache at each decoding phase."
 *
 * Position Renumbering Requirement:
 * > "if the cache has tokens [0, 1, 2, 3, 6, 7, 8], the assigned positions are [0, 1, 2, 3, 4, 5, 6]"
 *
 * WHY THIS MATTERS:
 * RoPE (Rotary Position Embeddings) encodes relative distances between tokens. If we used
 * original text positions [0,1,2,3,545,546,...,800] after reseeding, the model would see
 * HUGE position gaps that break attention calculations.
 *
...
TEST_CASE("RoPE Invariant: Clear+reseed produces contiguous positions") {
    ...
    if (max_pos_after_reseed == SINK_COUNT + TAIL_COUNT - 1) {
        INFO("✅ POSITIONS CONTIGUOUS: [0-" << max_pos_after_reseed << "] (no gaps)");
    } else {
        INFO("❌ POSITIONS NOT CONTIGUOUS: expected " << (SINK_COUNT + TAIL_COUNT - 1)
             << ", got " << max_pos_after_reseed);
    }
