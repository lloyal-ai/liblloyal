...
TEST_CASE("branch: create initializes state") {
  ...
  BranchHandle h = create(fake_ctx, fake_model, 0, 100, params, 512, nullptr, &store);
...
TEST_CASE("branch: create with null ctx/model returns invalid") {
  ...
  CHECK(create(nullptr, nullptr, 0, 0, params, 512, nullptr, &store) == INVALID_HANDLE);

  auto* fake_model = reinterpret_cast<llama_model*>(0x2000);
  CHECK(create(nullptr, fake_model, 0, 0, params, 512, nullptr, &store) == INVALID_HANDLE);

  auto* fake_ctx = reinterpret_cast<llama_context*>(0x1000);
  CHECK(create(fake_ctx, nullptr, 0, 0, params, 512, nullptr, &store) == INVALID_HANDLE);
}
...
TEST_CASE("branch: fork creates independent copy") {
  ...
  BranchHandle parent = create(fake_ctx, fake_model, 0, 50, params, 512, nullptr, &store);
...
TEST_CASE("branch: state accessors return correct values") {
  ...
  BranchHandle h = create(fake_ctx, fake_model, 5, 200, params, 512, nullptr, &store);
...
TEST_CASE("branch: RAII Branch auto-frees on destruction") {
  ...
  {
    Branch b = Branch::create(fake_ctx, fake_model, 0, 100, params, 512, nullptr, &store);
    CHECK(b.valid());
    raw_handle = b.handle();
...
TEST_CASE("branch: RAII Branch move semantics") {
  ...
  Branch b1 = Branch::create(fake_ctx, fake_model, 0, 100, params, 512, nullptr, &store);
...
TEST_CASE("branch: RAII Branch fork returns new Branch") {
  ...
  Branch parent = Branch::create(fake_ctx, fake_model, 0, 100, params, 512, nullptr, &store);
...
TEST_CASE("branch: get_logits returns nullptr before decode_and_capture") {
  ...
  BranchHandle h = create(fake_ctx, fake_model, 0, 0, params, 512, nullptr, &store);
  REQUIRE(h != INVALID_HANDLE);
...
TEST_CASE("branch: candidates_buffer is pre-allocated on fork") {
  ...
  BranchHandle parent = create(fake_ctx, fake_model, 0, 0, params, 512, nullptr, &store);
  REQUIRE(parent != INVALID_HANDLE);
...
TEST_CASE("branch: release resets has_logits flag") {
  ...
  BranchHandle h1 = create(fake_ctx, fake_model, 0, 0, params, 512, nullptr, &store);
...
TEST_CASE("branch: RAII Branch prune() invalidates handle") {
  ...
  Branch b = Branch::create(fake_ctx, fake_model, 0, 0, params, 512, nullptr, &store);
  REQUIRE(b.valid());
...
TEST_CASE("branch: RAII Branch move leaves source invalid") {
  ...
  Branch b1 = Branch::create(fake_ctx, fake_model, 0, 0, params, 512, nullptr, &store);
  BranchHandle h = b1.handle();
